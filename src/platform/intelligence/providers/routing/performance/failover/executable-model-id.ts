...
const DEFAULTS: Readonly<Record<string, string>> = {
  ...
  xai: "xai/grok-2",
  "provider.blackforestlabs": "blackforestlabs/flux-kontext-pro",
  blackforestlabs: "blackforestlabs/flux-kontext-pro",
};
...
const UNSAFE_WIRE: ReadonlySet<string> = new Set([
  ...
  "o1",
  "o3-mini",
  // Not available on current Gemini API keys (404) — prefer 2.5-flash.
  "gemini-2.0-flash",
...
const OPENAI_IMAGE_DEFAULT = "openai/gpt-image-1";
const GOOGLE_IMAGE_DEFAULT = "google/gemini-2.5-flash-image";
const BFL_IMAGE_DEFAULT = "blackforestlabs/flux-kontext-pro";
...
export function resolveExecutableModelId(
  ...
  if (!modelId?.trim()) {
    ...
    if (vendor === "blackforestlabs") return BFL_IMAGE_DEFAULT;
  ...
  // OpenAI image path — never dispatch retired DALL·E; prefer gpt-image-1.
  if (vendor === "openai") {
  ...
  if (vendor === "google") {
    if (
      wire.includes("imagen") ||
      UNSAFE_WIRE.has(wire) ||
      wire === "gemini-3-pro-image"
    ) {
  ...
  if (vendor === "blackforestlabs") {
    if (wire.includes("flux")) {
      // Prefer kontext-pro (matrix) over pro-1.1 when MI picks the latter.
      if (wire === "flux-pro-1-1" || wire === "flux-pro-1.1") {
        return BFL_IMAGE_DEFAULT;
      }
      return withVendorPrefix(providerId, wire);
    }
  }
