...
const createProject = asyncHandler(async (req: RequestUser, res) => {
  ...
  if (customer?.relationship_manager) {
    ...
    if (relationship_manager) {
      const csNotificationData = parseNotificationContent(NOTIFICATION_CONFIG.CS_PROJECT_CREATED.email_body, { Name: relationship_manager?.name || "Member" });
      EmailQueue.add("CS project created", {
        action: "PROJECT",
        data: commonTemplate({
          title: NOTIFICATION_CONFIG.CS_PROJECT_CREATED.email_subject,
          content: csNotificationData.text,
          name: relationship_manager?.name!,
          buttonText: csNotificationData.cta || "View Project",
          buttonLink: `${FRONTEND_URL}/project-logs/${create?._id.toString()}`,
        }),
        ...
        notification: new Notification({
          ...
          type: "PROJECT",
          action: `${FRONTEND_URL}/project-logs/${create._id}`,
          actionText: csNotificationData.cta || "view project",
          symbol: "🚀",
        }),
